//
// Extended tests for DbVu64 functionality
//
mod test_dbvu64_extended {
    use abyssiniandb::filedb::{FileDbParams, HashBucketsParam};
    use abyssiniandb::{DbXxx, DbXxxBase};

    #[test]
    fn test_put_get_vu64_edge_cases() {
        let db_name = "target/tmp/test_dbvu64_extended/test_put_get_vu64_edge_cases.abyssiniandb";
    ...
    #[test]
    fn test_bulk_put_get_vu64() {
        let db_name = "target/tmp/test_dbvu64_extended/test_bulk_put_get_vu64.abyssiniandb";
        let _ = std::fs::remove_dir_all(db_name);
        let db = abyssiniandb::open_file(db_name).unwrap();
        let mut db_map = db
            .db_map_vu64_with_params(
    ...
    #[test]
    fn test_bulk_delete_vu64() {
        let db_name = "target/tmp/test_dbvu64_extended/test_bulk_delete_vu64.abyssiniandb";
        let _ = std::fs::remove_dir_all(db_name);
        let db = abyssiniandb::open_file(db_name).unwrap();
        let mut db_map = db
            .db_map_vu64_with_params(
    ...
    #[test]
    fn test_includes_key_vu64() {
        let db_name = "target/tmp/test_dbvu64_extended/test_includes_key_vu64.abyssiniandb";
        let _ = std::fs::remove_dir_all(db_name);
        let db = abyssiniandb::open_file(db_name).unwrap();
        let mut db_map = db
            .db_map_vu64_with_params(
